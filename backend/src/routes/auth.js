...
router.post("/verify-otp", async (req, res) => {
  try {
    ...
    if (new Date() > user.otp_expiry) return res.status(400).json({ error: "OTP has expired. Please request a new one." });
    if (user.otp_attempts >= 3) return res.status(400).json({ error: "Maximum verification attempts exceeded. Please request a new OTP." });
    ...
    res.json({ success: true, message: "OTP verified successfully", token, user: user.toJSON() });
  } catch (err) {
...
router.post("/reset-password", async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ error: "User not found" });
    if (!user.reset_password_token || user.reset_password_token !== token) return res.status(400).json({ error: "Invalid reset code" });
    if (new Date() > user.reset_password_expiry) return res.status(400).json({ error: "Reset code has expired" });
...
router.post("/register", async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: "Registration successful! OTP sent to your email.", otpSent: true, email: user.email });
  } catch (error) {
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user.isActive && user.email_verified) return res.status(403).json({ error: "Account is disabled. Please contact support." });
    if (role === "admin" && user.role !== "admin") return res.status(403).json({ error: "This account does not have admin privileges" });
